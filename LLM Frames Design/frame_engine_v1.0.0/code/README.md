...
│   ├── config.yaml        # Configuration for the application (model, logging)
│   ├── frontend.py        # The interactive web UI
│   └── .env               # API key storage (must be created manually)
├── src/
...
-   **Key Features**:
    -   **LLM-Powered Analysis**: In Slot 1, this frame makes its own internal LLM call to perform a deep analysis of the student's message, determining their intent, understanding level, and relevance to the task. This rich context is the foundation of its intelligence.
    -   **Stateful Session Management**: It uses its `frame_memory` to track the conversation's phase, turn count, and student participation levels.
    -   **Dynamic Prompt Shaping**: Based on its analysis, it dynamically shapes the prompt for the main LLM, adding instructions to adapt to the student's level, manage the conversation phase, and encourage quiet students to participate.
...
The application requires a Google Gemini API key. Create a file named `.env` inside the `scripts/` directory.

```bash
# From within the 'code' directory:
echo 'GOOGLE_API_KEY="YOUR_API_KEY_HERE"' > scripts/.env
```
