...
export-env {
    ...
    let virtual_env = r#'/Users/olga/Olga's workspace/ETHZ SBS/Marty project/LLM Frames Design/frame_engine_v1.0.0/code/.venv'#
    let bin = r#'bin'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'backend-py3.11'# | is-empty) {
