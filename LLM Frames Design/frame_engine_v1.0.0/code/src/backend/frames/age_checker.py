...
The tone should be encouraging and simple, not condescending or overly complex.
...
class AgeCheckerFrame(Frame):
    """A frame that uses an LLM to validate age-appropriate language."""

    def __init__(self, target_age: int, llm_client: BaseChatModel):
        """Initializes the AgeCheckerFrame.
    ...
    async def validate_output(self, context: FrameContext) -> ValidationResult:
        ...
        if "false" in is_appropriate_str:
            return {
                ...
                "feedback": f"The language was not appropriate for a {self.target_age}-year-old. Please simplify your wording and use a more encouraging, less complex tone.",
            }
